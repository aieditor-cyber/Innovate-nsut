  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[var(--color-background-light)] dark:bg-[var(--color-background-dark)]">
      <div className="w-full max-w-md p-8 rounded-xl shadow-lg bg-[var(--color-surface-light)] dark:bg-[var(--color-surface-dark)] border border-[var(--color-border-light)] dark:border-[var(--color-border-dark)]">
        <h2 className="text-2xl font-bold mb-2 text-[var(--color-text-main)]">Create an account</h2>
        <p className="text-sm text-[var(--color-text-muted)] mb-6">Start using TerraVision</p>

        <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
          <div>
            <label className="block text-sm font-medium text-[var(--color-text-muted)]">Full name</label>
            <input type="text" placeholder="Jane Doe" className="mt-1 block w-full rounded-md border border-[var(--color-border-light)] bg-transparent px-3 py-2 text-[var(--color-text-main)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]" />
          </div>
          ...
          <button className="w-full py-2 rounded-md bg-[var(--color-primary)] text-white font-semibold hover:opacity-95">Create account</button>
        </form>

        <div className="mt-4 text-sm text-[var(--color-text-muted)]">
          Already have an account? <Link to="/login" className="text-[var(--color-primary)] font-semibold">Sign in</Link>
        </div>
      </div>
    </div>
  );
